...
// Helper to interact with Vercel Serverless Function proxy (now sends JWT token)
const upstashFetch = async (command: string, ...args: any[]) => {
    try {
        const key = args[0];
        const value = args[1]; // Only for SET
        ...
        if (!response.ok) {
            console.warn(`Data Proxy Error: ${response.status}`);
            return null;
        }
