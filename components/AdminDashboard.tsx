...
import { addReply, getAllAdmins, getAllStudents, deleteFeedback, deleteUser, updateUser, getChatHistory, getAllPlatformFeedback, updatePlatformFeedbackStatus, getUserFeedback, sendNotificationToUser, verifyUserDocument, removeUserDocument, saveChatSessionToUpstash, getAuthHeaders } from '../services/db';
...
import {
    ...
    MessageSquarePlus, CheckCircle, Bell, Send, ArrowLeft, FileText, Activity, User as UserIcon, Mail, Smartphone, Wand2, XCircle, ToggleRight, List, AlertTriangle, Lightbulb as IdeaIcon
} from 'lucide-react';
...
const PERMISSIONS: Record<AdminRole, Tab[]> = {
    ...
    'manager': ['inquiries', 'students', 'insights', 'chats', 'chat_insights', 'feedback_hub', 'settings'],
    'editor': ['settings'],
    'support': ['inquiries', 'students', 'chats']
};
...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ feedbackList: initialFeedbackList, onRefresh, onLogout, isLoading, currentUser, theme, toggleTheme }) => {
    ...
    const handleCreateAdmin = async () => {
        ...
        try {
            // Use secure admin-only endpoint instead of public register
            const response = await fetch('/api/admin/create', {
                method: 'POST',
                headers: getAuthHeaders(),
                body: JSON.stringify({ name: newAdminName, email: newAdminEmail, password: newAdminPass, adminRole: newAdminRole })
            });
            if (!response.ok) {
        ...
        } catch (e: any) { alert(e.message); } finally { setIsCreatingAdmin(false); }
    };
                                ...
                                {admins.map(admin => (
                                    <div key={admin.id} className="flex flex-col p-4 bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm relative group">
                                        <div className="flex items-center gap-4 mb-3">
                                            <div className="w-12 h-12 bg-indigo-100 dark:bg-indigo-900/30 rounded-xl flex items-center justify-center text-indigo-600 dark:text-indigo-400 font-bold text-lg">{admin.name.charAt(0)}</div>
                                            <div>
